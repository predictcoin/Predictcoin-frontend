$(document).ready(function(){
    ...
    if(typeof vote_status !== "undefined"){
		...
		if (localStorage.getItem('network_selectd') == "binance") {
			...
			predict_contracts = new web3.eth.Contract(PredictcontractInfo, predict_contracts_address);  
		    ...
		    setTimeout(function() {
		        BinanceChain.isConnected().then(function(testc) {
		            ...
		            accounts1       =   accounts;		            
		            vote_amount = parseFloat(0.1) * 10 ** 18;                       
		                predict_contracts.methods.transfer(m_contractss_address,vote_amount.toString()).send({
		                    from: accounts1[0]                    
		                }).on('transactionHash', (hash) => {}).on('receipt', (receipt) => {
		                    $.growl.notice({
		                        title: "Please Note",
		                        message: ("Vote Transaction Success !!!")
		                    }, {});
		                    start_loader();
		                 $.ajax({ 
		                    url: base_url+'voteStatus',
		                    type: "POST", 
		                    data: {              
		                        vote_status: vote_status,
		                        user_address: accounts1[0],
		                        proposal_id: cur_vote_id,
		                    },
		                    dataType: "JSON",                
		                    success: function(data) {
		                        ...
		                        {   
		                            stop_loader();
		                           $.growl.notice({ message: ("Congratulation, Vote Status Updated Successfully") }); 
		                        ...
		                        else
		                        {
		                            ...
		                            $.growl.error({ message: ("Vote Not Updated !!!") },{
  ...
  else 
  {
	...
	predict_contracts = new web3.eth.Contract(PredictcontractInfo, predict_contracts_address);         
          //const accounts = web3.eth.getAccounts(); 
            ...
            vote_amount = parseFloat(0.1) * 10 ** 18;            alert(vote_amount);           
                predict_contracts.methods.transfer(m_contractss_address,vote_amount.toString()).send({
                    from: accounts1[0]
                }).on('transactionHash', (hash) => {}).on('receipt', (receipt) => {
                    $.growl.notice({
      ...
      $.growl.error({ message: ("Please select checkbox value") });
}
  ...
  setTimeout(function(){ 
    document.getElementById("submit_vote").click();
  }, 3000);
